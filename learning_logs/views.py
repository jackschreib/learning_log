...
def topic(request, topic_id):
    """Show a single topic and all its entries (owner-only)."""
...
def new_topic(request):
    """Add a new topic and associate it with the current user."""
...
def new_entry(request, topic_id):
    """Add a new entry for a particular topic (owner-only)."""
    topic = Topic.objects.get(id=topic_id)
    if topic.owner != request.user:
        raise Http404
...
def edit_entry(request, entry_id):
    """Edit an existing entry (owner-only via its topic)."""
